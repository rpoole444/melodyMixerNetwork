    ...
    setTracks(
      [...show.songs]
        ...
        .map((song) => ({
          ...
          artist: song.artist || "",
          album: song.album || "",
          length: song.duration ? String(song.duration) : "",
            ...
            <div className="mt-5 grid gap-4 md:grid-cols-2">
              ...
              <div className="md:col-span-2">
                ...
                <div className="mt-2 flex flex-col gap-3 rounded-md border border-zinc-700 bg-zinc-950 p-4 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <input ref={fullShowInputRef} type="file" accept="audio/*" className="hidden" onChange={(e) => handleFullShowFileChange(e.target.files)} />
                  <button type="button" onClick={() => fullShowInputRef.current?.click()} className="rounded-md border border-white/15 px-4 py-3 font-semibold text-white hover:border-amber-300">
                    Select Full Show
                  </button>
                </div>
              </div>
                      ...
                      <article key={audioFile.id} className="p-5">
                        <div className="flex items-start justify-between gap-4">
                          <div>
                            ...
                            <p className="text-sm text-zinc-400">{audioFile.artist || "Unknown Artist"} {audioFile.album && `- ${audioFile.album}`}</p>
                          </div>
                    ...
                    {expandedTrackId === track.id && (
                      <div className="mt-4 rounded-md border border-white/10 bg-zinc-950 p-4">
                        <dl className="grid gap-3 text-sm sm:grid-cols-2">
                          <div>
                            <dt className="text-zinc-500">Album / Segment</dt>
                            <dd className="text-zinc-200">{track.album || "Single"}</dd>
                          </div>
                          <div>
                            <dt className="text-zinc-500">Audio File</dt>
                            <dd className="text-zinc-200">{track.fileName || "No file attached"}</dd>
                          </div>
                        </dl>
