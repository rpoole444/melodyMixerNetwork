    ...
    <main className="min-h-screen bg-zinc-950 text-white">
      ...
      <section className="mx-auto w-full max-w-7xl px-4 py-10 sm:px-6">
        {user?.role !== "admin" ? (
          ...
          <>
            <div>
              <p className="text-sm font-semibold uppercase tracking-[0.3em] text-amber-300">Station People</p>
              <h1 className="mt-3 text-4xl font-semibold">Build a trusted host team.</h1>
              <p className="mt-3 max-w-3xl text-zinc-300">Invite musicians and selectors, see what they have created, and pause access without deleting their work.</p>
            </div>
            ...
            <section className="mt-8 grid gap-4 sm:grid-cols-3">
              <div className="rounded-md border border-white/10 bg-zinc-900 p-5"><p className="text-sm text-zinc-400">Active Hosts</p><p className="mt-2 text-3xl font-semibold">{activeHosts}</p></div>
              <div className="rounded-md border border-white/10 bg-zinc-900 p-5"><p className="text-sm text-zinc-400">Paused Hosts</p><p className="mt-2 text-3xl font-semibold">{hosts.length - activeHosts}</p></div>
              <div className="rounded-md border border-white/10 bg-zinc-900 p-5"><p className="text-sm text-zinc-400">Open Invites</p><p className="mt-2 text-3xl font-semibold">{invitations.filter((invite) => invite.status === "active").length}</p></div>
            </section>
            ...
            <form onSubmit={createInvitation} className="mt-8 rounded-md border border-white/10 bg-zinc-900 p-5">
              ...
              <div className="mt-5 grid gap-4 md:grid-cols-3">
                ...
                <label className="text-sm font-medium text-zinc-200">Show Idea Or Note<input value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Late-night soul, local jazz..." className="mt-2 w-full rounded-md border border-zinc-700 bg-zinc-950 px-3 py-3 text-white" /></label>
              </div>
              <button type="submit" disabled={busyId === 0} className="mt-4 w-full rounded-md bg-amber-300 px-4 py-3 font-semibold text-zinc-950 hover:bg-amber-200 disabled:opacity-60">{busyId === 0 ? "Creating Invite..." : "Create Invite"}</button>
            </form>
