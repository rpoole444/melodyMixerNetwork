      ...
      <section className="mx-auto w-full max-w-7xl px-4 py-10 sm:px-6">
        {user?.role !== "admin" ? (
          <div className="rounded-xl border border-signal/40 bg-signal/30 p-6">
            ...
            <p className="mt-3 max-w-2xl text-paper/70">Hosts submit shows. Admins arrange approved shows into a broadcast day.</p>
          </div>
        ) : (
          <>
            <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
              <div>
                <p className="hf-kicker">Human Frequency transmission</p>
                <h1 className="mt-3 text-5xl">Build a full day on the dial.</h1>
              ...
              <div className="flex flex-col gap-3 sm:flex-row">
                <label className="text-sm font-medium text-paper/80" htmlFor="programming-day">
                ...
                <button type="button" onClick={loadShows} className="self-end rounded-xl border border-white/15 px-4 py-3 font-semibold text-white hover:border-signal">
                  Refresh
                </button>
              </div>
            </div>
            ...
            <section className="mt-8 grid gap-4 md:grid-cols-5">
              <div className="rounded-xl border border-white/10 bg-ink-soft p-5">
                <p className="text-sm text-paper/50">Coverage</p>
                <p className="mt-2 text-3xl font-semibold">{coveragePercent}%</p>
                <p className="mt-1 text-sm text-[#ff9a82]">{formatDuration(coverageSeconds)} programmed</p>
              </div>
              <div className="rounded-xl border border-white/10 bg-ink-soft p-5">
                <p className="text-sm text-paper/50">Scheduled Shows</p>
                <p className="mt-2 text-3xl font-semibold">{scheduledShows.length}</p>
                <p className="mt-1 text-sm text-[#ff9a82]">on this day</p>
              </div>
              <div className="rounded-xl border border-white/10 bg-ink-soft p-5">
                <p className="text-sm text-paper/50">Stream Packages</p>
                <p className="mt-2 text-3xl font-semibold">{queuedCount}</p>
                <p className="mt-1 text-sm text-[#ff9a82]">queued for provider handoff</p>
              </div>
              <div className="rounded-xl border border-white/10 bg-ink-soft p-5">
                <p className="text-sm text-paper/50">Open Gaps</p>
                <p className="mt-2 text-3xl font-semibold">{gaps.length}</p>
                <p className="mt-1 text-sm text-[#ff9a82]">15+ minute openings</p>
              </div>
              <div className={`rounded-xl border p-5 ${overlaps.length > 0 ? "border-red-400/50 bg-red-950/30" : "border-white/10 bg-ink-soft"}`}>
                <p className="text-sm text-paper/50">Conflicts</p>
            ...
            {overlaps.length > 0 && (
              <section className="mt-6 rounded-xl border border-red-400/50 bg-red-950/30 p-5">
            ...
            <section className="mt-8 grid gap-6 lg:grid-cols-[1.3fr_0.7fr]">
              <div className="rounded-xl border border-white/10 bg-ink-soft">
                <div className="border-b border-white/10 p-5">
                  <h2 className="text-xl font-semibold">24-Hour Clock</h2>
                  <p className="mt-1 text-sm text-paper/50">Shows are positioned by scheduled start time and estimated duration.</p>
                </div>
                <div className="relative">
                  {HOURS.map((hour) => (
                    <div key={hour} className="grid min-h-20 grid-cols-[80px_1fr] border-b border-white/10 last:border-b-0">
                      <div className="border-r border-white/10 p-3 font-mono text-sm text-paper/35">{String(hour).padStart(2, "0")}:00</div>
                  ...
                  <div className="pointer-events-none absolute left-[80px] right-0 top-0 h-full">
                    {clockItems.map((item) => {
                      ...
                      <div
                        key={item.show.id}
                        className={`absolute left-3 right-3 overflow-hidden rounded-xl border p-3 ${hasConflict ? "border-red-400/70 bg-red-500/25" : "border-signal/40 bg-signal/15"}`}
                        style={{
                      ...
                      >
                        ...
                        <p className="truncate text-xs text-[#ffd5c9]">
              ...
              <aside className="space-y-6">
                <section className="rounded-xl border border-white/10 bg-ink-soft">
                  <div className="border-b border-white/10 p-5">
                    ...
                    <p className="mt-1 text-sm text-paper/50">Approved shows that need a broadcast slot.</p>
                  </div>
                  <div className="divide-y divide-white/10">
                    {readyShows.length === 0 ? (
                      <p className="p-5 text-sm text-paper/50">No ready unscheduled shows.</p>
                    ) : (
                      readyShows.map((show) => (
                        <article key={show.id} className="p-5">
                          <h3 className="font-semibold">{show.name}</h3>
                          <p className="mt-1 text-sm text-paper/50">{show.host_name} - {formatDuration(showDurationSeconds(show))}</p>
                          <div className="mt-4 grid gap-3">
                            <input
                              ...
                              className="w-full rounded-xl border border-paper/15 bg-ink px-3 py-3 text-white outline-none focus:border-signal"
                            />
                            <button
                              type="button"
                              onClick={() => scheduleShow(show)}
                              disabled={busyId === show.id}
                              className="rounded-xl bg-signal px-4 py-3 font-semibold text-ink hover:bg-[#ff7658] disabled:cursor-wait disabled:opacity-60"
                            >
                              Place On Clock
                            </button>
                          </div>
                ...
                <section className="rounded-xl border border-white/10 bg-ink-soft">
                  <div className="border-b border-white/10 p-5">
                    ...
                    <p className="mt-1 text-sm text-paper/50">Open space left in this broadcast day.</p>
                  </div>
                  <div className="divide-y divide-white/10">
                    {gaps.length === 0 ? (
                      <p className="p-5 text-sm text-paper/50">No 15+ minute gaps.</p>
                    ) : (
                      gaps.map((gap) => (
                        <div key={`${gap.start}-${gap.end}`} className="p-4">
                          <p className="font-medium">{minuteLabel(gap.start)} - {minuteLabel(gap.end)}</p>
                          <p className="text-sm text-paper/50">{formatDuration((gap.end - gap.start) * 60)} open</p>
                        </div>
                      ))
                    )}
                  </div>
